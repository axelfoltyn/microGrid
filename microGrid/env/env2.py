...
class MicroGrid2(Environment, gym.Env):
    def __init__(self, rng, reduce_qty_data=None, length_history=None, start_history=None):
        ...
        print("Average consumption per day train (kWh): {}".format(
            np.sum(self.consumption_train) / self.consumption_train.shape[0] * 24))
        print("Average consumption per day valid (kWh): {}".format(
            np.sum(self.consumption_valid) / self.consumption_valid.shape[0] * 24))
        print("Average consumption per day test (kWh): {}".format(
        ...
        if (self._reduce_qty_data != 1):
            for season in range(4):
                self.production_train[season * nd_one_seas:(season + 1) * nd_one_seas] = np.tile(self.production_train[
                                                                                                 int((season + (
                ...
                self.production_valid[season * nd_one_seas:(season + 1) * nd_one_seas] = np.tile(self.production_valid[
                                                                                                 int((season + (
                                                                                                             self._start_history + 0.) / self._reduce_qty_data) * nd_one_seas):int(
                                                                                                     (season + (
                                                                                                                 self._start_history + 1.) / self._reduce_qty_data) * nd_one_seas)],
        ...
        print("Average production per day valid (kWh): {}".format(
            np.sum(self.production_valid) / self.production_valid.shape[0] * 24))
        print("Average production per day test (kWh): {}".format(
    ...
    def step(self, action):
        ...
        if (Energy_needed_from_battery > 0):
            ...
            if (self._last_ponctual_observation[0] * self.battery_size > Energy_needed_from_battery):
                ...
                self._last_ponctual_observation[0] = self._last_ponctual_observation[
        ...
        self.counter += 1
        res_train, res_test = self.my_reward()
        done = self.counter + 24 >= len(self.production_norm) if self._pred == 1 else self.counter >= len(self.production_norm)
        info = {}
        return self._last_ponctual_observation, np.sum(list(res_train.values())), done, info
    ...
    def act(self, action):
        ...
        if (Energy_needed_from_battery > 0):
            ...
            if (self._last_ponctual_observation[0] * self.battery_size > Energy_needed_from_battery):
                ...
                #    0] * self.battery_size) * 2  # 2euro/kWh
                self._last_ponctual_observation[0] = 0
        elif (Energy_needed_from_battery < 0):
            # Surplus of energy --> load the battery
            self.dict_param["waste_energy"] = max(0, (self._last_ponctual_observation[0] * self.battery_size
        ...
        self.counter += 1
        res_train, res_test = self.my_reward()
        return np.sum(list(res_train.values()))
